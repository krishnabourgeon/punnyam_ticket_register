...
class _BookRow {
  ...
  final leavesCtrl = TextEditingController(); 
...
class _NewBookScreenState extends State<NewBookScreen> {
  ...
  // ── Dummy pooja -> leaf count mapping ───────────────────────────────────────
  // TODO: replace with real data once an API/provider is available.
  ...
  List<String> get _poojaList => _poojaLeafCounts.keys.toList();
  ...
  _BookRow _newRow() {
    final row = _BookRow();
    // Recalculate No of Books whenever From/To changes for this row.
    row.fromCtrl.addListener(() => _recalcBooks(row));
    row.toCtrl.addListener(() => _recalcBooks(row));
    return row;
  }

  // Recomputes row.noOfBooks from From/To and the leaves-per-book for the
  // selected pooja. Left as null (shown as "—") until all three are known.
  void _recalcBooks(_BookRow row) {
    final from = int.tryParse(row.fromCtrl.text.trim());
    final to = int.tryParse(row.toCtrl.text.trim());
    final leavesPerBook = _poojaLeafCounts[row.pooja];
  ...
  // When a pooja is picked for a row, auto-fill the leaf count from dummy data
  // and recompute the derived No of Books.
  void _onPoojaSelected(_BookRow row, String? value) {
    setState(() {
      row.pooja = value;
      final leafCount = _poojaLeafCounts[value];
      row.leavesCtrl.text = leafCount != null ? leafCount.toString() : '';
    });
    _recalcBooks(row);
  }
  ...
  void _onSubmit() {
    ...
    final fractionalRows = <int>[];
    for (int i = 0; i < _rows.length; i++) {
      if (_rows[i].hasFractionalBooks) fractionalRows.add(i + 1);
    }

    if (fractionalRows.isNotEmpty) {
      final label = fractionalRows.length == 1
          ? 'Receipt ${fractionalRows.first}'
          : 'Receipts ${fractionalRows.join(", ")}';
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          behavior: SnackBarBehavior.floating,
          ...
          content: Text(
            '$label: leaf range doesn\'t divide evenly into whole Receipts. Adjust From/To.',
    ...
    // TODO: pass _rows data to API / provider
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        behavior: SnackBarBehavior.floating,
        ...
        content: Text(
          '${_rows.length} Receipt${_rows.length > 1 ? 's' : ''} saved successfully!',
          style: GoogleFonts.poppins(
            color: Colors.white,
            fontSize: 14.sp,
            fontWeight: FontWeight.w600,
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              SizedBox(width: 14.w),
              // FIX: wrap the title Column in Expanded so it takes only the
              // remaining space after the back button and badge are laid
              // out, instead of sizing itself to its natural (unconstrained)
              // width. Combined with ellipsis overflow, this prevents the
              // RenderFlex overflow on narrower screens / larger text scale.
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // No of Leaves (auto-filled, read-only) + No of Books (derived)
          Row(
            children: [
    ...
    return Container(
      ...
      child: DropdownButtonFormField<String>(
        ...
        items: _poojaList
            ...
            .toList(),
        onChanged: (v) => _onPoojaSelected(row, v),
        validator: (v) => v == null ? 'Select a pooja' : null,
      ),
    );
  }
  ...
  // ─── LEAVES FIELD (auto-filled, read-only) ─────────────────────────────────
  Widget _buildLeavesField(_BookRow row) {
    final hasValue = row.leavesCtrl.text.isNotEmpty;
    return Container(
      decoration: BoxDecoration(
        gradient: LinearGradient(
